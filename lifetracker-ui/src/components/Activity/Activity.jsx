...
export default function Activity({ user, setAppState }) {
  ...
  const title = isAuthenticated
    ...
    : "Please login to see the portal";

  const [averageCalories, setAverageCalories] = useState(0.0);
  const getCaloriesData = async () => {
    try {
      const res = await axios.post(
        "https://lifetracker-server.onrender.com/nutritionavgcalories",
        { userId: user?.id }
      );
      setAverageCalories(
        res.data.nutrition.average === null ? 0.0 : res.data.nutrition.average
      );
    } catch (error) {
      console.log(error);
    }
